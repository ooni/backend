        ...
        for row in fdindex:
            ...
            if 'canned' in row:
                if {_['textname'] for _ in row['canned']} & to_del:
                    raise RuntimeError('Destination index lists files to delete', row['filename'])
...
def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: delete reports from canned and reports-tgz files')

    g1 = p.add_mutually_exclusive_group(required=True)
    g1.add_argument('--canned', metavar='DIR', type=dirname, help='Path to .../private/canned')
    g1.add_argument('--reports-tgz', metavar='DIR', type=dirname, help='Path to .../private/reports-tgz')
    ...
    g2.add_argument('--dst', metavar='DIR', type=dirname, help='Path for repacked archives')

    g3 = p.add_mutually_exclusive_group(required=True)
    g3.add_argument('--files-from', metavar='FILE', type=filename, help='Get names to delete from FILE')
...
def main():
    ...
    if opt.list:
        ...
        if not os.path.exists(index_dst):
            canned_report_delete(src_root, index, affected, to_del, opt.dst, index_dst, compress)
        else:

...
def filelist_from_canndx(canned_index):
    ...
    for caninfo in canned_index:
        if 'canned' in caninfo:
            ...
            filelist.append({k: caninfo[k] for k in (
                'text_crc32', 'text_sha1', 'text_size', 'textname')})
    return filelist
...
def tar_reports_raw(tar_dir, reports_root, canned_root, bucket):
    ...
    if os.path.exists(tarred_index_fname):
        with gzip.GzipFile(tarred_index_fname, 'rb') as gzfd:
            ...
            if tarred_index['canned'] == filelist_from_canndx(canned_index): # does not respect order
    ...
    if len(reports) == 0:
        ...
        for caninfo in canned_index:
            if 'canned' in caninfo:
                with ScopedTmpdir(dir=reports_dir) as tmpdir:
                    subprocess.check_call(['tar', '--extract', '-I', 'lz4',
                        '--file', os.path.join(canned_root, caninfo['filename']),
                        '--directory', tmpdir])
                    for c in caninfo['canned']:
                        assert c['textname'].startswith(prefix)
                        os.rename(os.path.join(tmpdir, c['textname']), os.path.join(reports_dir, c['textname'][len(prefix):]))
                ...
                with tempfile.NamedTemporaryFile(dir=reports_dir) as fdout, \
                     open(os.path.join(canned_root, caninfo['filename']), 'rb') as fdin:
        ...
        if canned_reports != reports:
            raise RuntimeError('private/reports-raw and private/canned index Mismatch', bucket)
    ...
    with tempfile.NamedTemporaryFile(dir=tar_dir) as fdout:
        subprocess.check_call(['tar', '--create', '--gzip', '--file', fdout.name,
            '--directory', reports_root, bucket])
        os.link(fdout.name, out)
    os.chmod(out, 0444)
    ...
    print 'Writing index for {} to {}'.format(tarred_index['filename'], tarred_index_fname)
    with tempfile.NamedTemporaryFile(dir=tar_dir) as fdout:
        with closing(gzip.GzipFile(filename='{}.index.json'.format(bucket),
                mtime=tarred_index['file_mtime'], mode='wb', fileobj=fdout)) as gzout:
...
def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: create private/reports-tgz from private/reports-raw or private/canned')
    p.add_argument('--bucket', metavar='BUCKET', help='Bucket to check', required=True)
    p.add_argument('--reports-tgz', metavar='DIR', type=dirname, help='Path to .../private/reports-tgz', required=True)
    p.add_argument('--reports-raw', metavar='DIR', type=dirname, help='Path to .../private/reports-raw', required=True)
    p.add_argument('--canned', metavar='DIR', type=dirname, help='Path to .../private/canned', required=True)
    opt = p.parse_args()

...
def gzip_canning_index(fname):
    ...
    with open(fname) as textin, \
         tempfile.NamedTemporaryFile(prefix='tmpcng', dir=outdir) as rawout:
        with closing(gzip.GzipFile(filename=basename, mtime=mtime, mode='wb', fileobj=rawout)) as textout:
            for line in textin:

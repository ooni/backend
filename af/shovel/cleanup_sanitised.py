...
def cleanup_sanitised(sanitised_root, autoclaved_root, bucket, s3_ls):
    ...
    with gzip.GzipFile(os.path.join(autoclaved_root, bucket, autoclaving.INDEX_FNAME)) as fd:
    ...
    for fname, size in listdir_filesize(os.path.join(sanitised_root, bucket)):
        ...
        if s3_size.get(idfname) == size and idfname in autoclaved_reports:
            ...
            print idfname, 'FS:', size, 'S3:', s3_size.get(idfname), 'autoclaved:', idfname in autoclaved_reports

def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: remove files from public/sanitised that are stored in public/autoclaved and listed in s3')
    ...
    p.add_argument('--santoken', metavar='FILE', type=file, help='Token for checked sanitised directory', required=True)
    opt = p.parse_args()

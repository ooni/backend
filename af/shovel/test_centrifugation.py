        ...
        for rowno in xrange(9):
            for ver in xrange(1 << rowno):
                ...
                with self.conn, self.conn.cursor() as c:
                    ...
                    c.execute('CREATE TEMPORARY TABLE bad{} (tbl text, code_ver integer, datum bytea) ON COMMIT DROP'.format(self.table))
...
class TestPartialReprocessing(unittest.TestCase):
    ...
    def take_part_of_bucket(acroot, bucket):
        ...
        httpreq_blob = sorted(_ for _ in files if '-http_requests-' in _ and _.endswith('.lz4'))
        webcon_blob = sorted(_ for _ in files if '-web_connectivity-' in _ and _.endswith('.lz4'))
        webcon_tar = sorted(_ for _ in files if _.startswith('web_connectivity.') and _.endswith('.tar.lz4')) # OONI Probe
        ...
        add = {webcon_tar[2]}
        files = {_ for _ in files if _.endswith('.lz4') and (_ not in questionable or _ in to_keep)}
    ...
    def test_reprocessing(self):
        ...
        self.mkdtemp(bucket)
        for f in files:
            os.symlink(os.path.join(acroot, bucket, f), os.path.join(self.tmpdir, bucket, f))
        ...
        with self.conn, self.conn.cursor() as c:
            c.execute("""UPDATE autoclaved SET code_ver = 0
        ...
        self.retry_centrifugation()
        with self.conn, self.conn.cursor() as c:
            self.assertEqual(magic, self.get_magic_numbers(c, '2017-08-28/web_connectivity.00.tar.lz4'))
        ...
        addendum = add.pop()
        shutil.copyfile(os.path.join(acroot, bucket, addendum), os.path.join(self.tmpdir, bucket, addendum))
        ...
        shutil.copyfile(os.path.join(acroot, bucket, alter), os.path.join(self.tmpdir, bucket, alter))
        with open(os.path.join(self.tmpdir, bucket, alter), 'a') as fd:
            fd.write(LZ4_EMPTY_FRAME)
        self.make_bucket_index(acroot, bucket, files | {addendum, alter}, {
            addendum: self.calc_cksum(os.path.join(self.tmpdir, bucket, addendum)),
        ...
        self.retry_centrifugation()
        with self.conn, self.conn.cursor() as c:
            self.assertEqual(magic, self.get_magic_numbers(c, '{}/{}'.format(bucket, alter)))

    def get_magic_numbers(self, c, filename):
        c.execute('SELECT autoclaved_no FROM autoclaved WHERE filename = %s', [filename])
        autoclaved_no = sorted(_[0] for _ in c)
        c.execute('''SELECT report_no
            FROM autoclaved
            JOIN report USING (autoclaved_no)
            WHERE filename = %s''', [filename])
        report_no = sorted(_[0] for _ in c)
        c.execute('''SELECT msm_no
            FROM autoclaved
            JOIN report USING (autoclaved_no)
    ...
    def call_centrifugation(self, msg):
        ...
        start = time.time()
        rc = subprocess.call(['./centrifugation.py',
            '--start', '2017-08-28T00:00:00', '--end', '2017-08-29T00:00:00',
            '--autoclaved-root', self.tmpdir,
            '--postgres', PG])
        end = time.time()
        print ' ^^^ centrifugation.py: rc = {:d}, duration = {:1f} sec'.format(rc, end - start)
        return rc, end - start
    ...
    def make_bucket_index(self, acroot, bucket, files, cksum):
        ...
        with gzip.GzipFile(os.path.join(acroot, bucket, 'index.json.gz'), 'r') as src, \
             gzip.GzipFile(os.path.join(self.tmpdir, bucket, 'index.json.gz'), 'w') as dst:
                ...
                if 'file"' in line:
                    ...
                    if doc['type'] == 'file':
                        ...
                        if not use:
                            ...
                            doc['file_size'], doc['file_crc32'], doc['file_sha1'] = cksum[filename]

...
def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: originas.bz2 -> postgres/table')
    p.add_argument('--originas', metavar='FILE', help='path to originas.bz2', required=True)
...
def main():
    ...
    with psycopg2.connect(opt.postgres) as conn, conn.cursor() as c:
        ...
        c.execute('CREATE TEMPORARY TABLE tmp (LIKE {} INCLUDING DEFAULTS)'.format(opt.table))

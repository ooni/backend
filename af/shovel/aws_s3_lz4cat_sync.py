...
def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: tar -I lz4 --extract *.tar.lz4 | aws s3 sync')
    p.add_argument('--src', metavar='SRC_PATH', help='Source to decompress *.lz4 files from', required=True)
    p.add_argument('--s3-bucket', metavar='BUCKET', help='Destination S3 bucket upload files to', required=True)
...
def aws_s3_lz4cat_sync(src, s3_bucket, s3_prefix):
    ...
    extra = aws_report.viewkeys() - ls_report.viewkeys() # FIXME: update to `viewitems()` when `src_size` is fixed for report
    if extra:
        # FIXME: that should be updated to respect some sort of mtime when pipeline-based PII scrubbing is rolled out
        raise RuntimeError('S3 bucket has unknown files', extra) # files with mismatching size also trigger that

    missing = ls_report.viewkeys() - aws_report.viewkeys() # FIXME: update to `viewitems()` when `src_size` is fixed for report
    file_to_unpack = {}
    ...
    for filename, missing_textname in file_to_unpack.iteritems():
        with ScopedTmpdir(prefix='oolz42s3') as tmpdir: # OO lz4 to s3 :-)
            ...
            if os.path.getsize(filepath) != ls_file[filename]:
                raise RuntimeError('Unexpected size of compressed file', filepath, os.path.getsize(filepath), ls_file[filename])

            if filename.endswith('.tar.lz4'):
                check_call(['tar', '--use-compress-program', 'lz4', '--directory', tmpdir, '--extract', '--file', os.path.join(src, filename)])
            elif filename.endswith(('.json.lz4', '.yaml.lz4')):
                os.mkdir(os.path.join(tmpdir, date))
                destname = filename[:-len('.lz4')]
                check_call(['lz4', '--decompress', os.path.join(src, filename), os.path.join(tmpdir, date, destname)])
            else:
                raise RuntimeError('Unexpected filename in the directory', src, filename)
            ...
            check_call(['aws', 's3', 'cp', '--recursive', os.path.join(tmpdir, date), 's3://{}/{}'.format(s3_bucket, s3_prefix)])

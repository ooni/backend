...
import json
import os
from base64 import b64decode # NB b64encode != str.encode('base64'), b64encode does not add newlines
...
def get_repeated_sha1(files):
    ...
    env = {'LC_ALL': 'C'}
    with ScopedPopen(['sort'], stdin=PIPE, stdout=PIPE, env=env) as sort, \
         ScopedPopen(['uniq', '--repeated'], stdin=sort.stdout, stdout=PIPE, env=env) as uniq:
        for fname in files:
...
def text_sha512(blob, text):
    ...
    with ScopedPopen(cmd, stdin=open(blob['filename']), stdout=PIPE) as proc:
        for c in iter(functools.partial(proc.stdout.read, 131072), ''):
            h.update(c)
        if proc.wait() != 0:
            raise RuntimeError('cmd failed with non-zero returncode', cmd, proc.returncode)
    return h.digest()

def parse_args():
    p = argparse.ArgumentParser(description='ooni-pipeline: private/canned -> duplicate reports')
    p.add_argument('--canned-root', metavar='DIR', type=dirname, help='Path to .../private/canned')
    p.add_argument('--postgres', metavar='DSN', help='libpq data source name')
    return p.parse_args()

def repeated_to_stdout(it):
    for blob, text in it:
        print text['text_sha1'], text_sha512(blob, text).encode('base64'), text['textname'], blob['filename']

def repeated_to_postgres(it, pgconn):
    with pgconn.cursor() as c:
        c.execute('''CREATE TEMPORARY TABLE dup (
                textname text UNIQUE NOT NULL,
                orig_sha1 sha1 NOT NULL,
                orig_sha512 sha512 NOT NULL
            ) ON COMMIT DROP''')
        with closing(PGCopyFrom(pgconn, 'dup', columns=('textname', 'orig_sha1', 'orig_sha512'))) as w:
            for blob, text in it:
                orig_sha1 = b64decode(text['text_sha1'])
                orig_sha512 = text_sha512(blob, text)
                w.write('{}\t{}\t{}\n'.format(
                    pg_quote(text['textname']), pg_binquote(orig_sha1), pg_binquote(orig_sha512)))

        # no changed hashes
        c.execute('''SELECT COUNT(*) FROM repeated_report rr JOIN dup ON (
...
def main():
    opt = parse_args()
    if opt.canned_root:
        os.chdir(opt.canned_root) # the easiest way to handle relative path is to avoid handling

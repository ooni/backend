...
def cleanup_reports_raw(reports_raw_root, bucket, texts):
    ...
    for fname, size in ls_raw:
        ...
        if (idfname, size) not in texts:
            raise RuntimeError('reports-raw have a file that is not canned & tgz\'ed', bucket, fname, size)
    for fname, size in ls_raw:
        os.unlink(os.path.join(reports_raw_root, bucket, fname))

def parse_args():
    # that was prievo
    p = argparse.ArgumentParser(description='ooni-pipeline: remove files from private/reports-raw that are stored as `canned` and `reports-tgz`')
    ...
    p.add_argument('--canned-index', metavar='FNAME', type=jsonl_gz, help='Path to .../canned/YYYY-MM-DD/index.json.gz', required=True)
    p.add_argument('--reports-tgz-index', metavar='FNAME', type=json_gz, help='Path to .../reports-tgz/YYYY-MM-DD.index.json.gz', required=True)
    opt = p.parse_args()
    return opt
...
def main():
    ...
    for textname, _ in texts:
        if not textname.startswith(prefix):
            raise RuntimeError('Index textname prefix does not match bucket', bucket, textname)

...
from daphn3 import Daphn3Protocol
from daphn3 import read_pcap, read_yaml
...
if get_settings().daphn3_yaml_file != "":
    daphn3Steps = read_pcap(Settings.daphn3_yaml_file)

elif get_settings().daphn3_pcap_file != "":
    daphn3Steps = read_yaml(Settings.daphn3_pcap_file)
...
class Daphn3ServerProtocol(Daphn3Protocol):
    def nextStep(self):
        ...
        if self.current_step >= (len(self.steps) - 1):
            log.info("Reached the end of the state machine")
            log.info("Censorship fingerpint bisected!")
            step_idx, mutation_idx = self.factory.mutation
            log.info(f"step_idx: {step_idx} | mutation_id: {mutation_idx}")

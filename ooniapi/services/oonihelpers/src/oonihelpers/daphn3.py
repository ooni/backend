...
def daphn3Mutate(steps, step_idx, mutation_idx):
    ...
    for idx, step in enumerate(steps):
        if idx == step_idx:
            step_string = list(step.values())[0]
...
class Daphn3Protocol(protocol.Protocol):
    ...
    def _current_step_data(self):
        ...
        return list(mutated_step[self.current_step].values())[0]
    ...
    def sendPayload(self):
        ...
        if current_step_role == self.role:
            log.info(f"In a state to do shit {current_step_data}")
            self.transport.write(current_step_data)
            self.nextStep()
        else:
            log.info("Not in a state to do anything")
    ...
    def dataReceived(self, data):
        current_step_role = list(self.steps[self.current_step].keys())[0]
        ...
        expected_data_in_this_state = len(list(self.steps[self.current_step].values())[0])
    ...
    def nextMutation(self):
        ...
        if c_mutation_idx >= (len(list(self.steps[c_step_idx].values())[0]) - 1):

...
def test_create_fetch_archive(client, client_with_user_role, client_with_admin_role):
    say = print
    ...
    say("Empty name")
    r = client_with_user_role.post("/api/_/ooni_run/create", json=z)
    ...
    say("fetch latest")
    r = client_with_user_role.get(f"/api/_/ooni_run/fetch/{ooni_run_link_id}")
    assert r.status_code == 200, r.json()
    assert r.json()["v"] == 1, r.json()
    exp_fetch_fields = [
        "archived",
    ...
    r = client_with_user_role.get("/api/_/ooni_run/list")
    assert r.status_code == 200, r.json()
    assert r.json()["v"] == 1, r.json()
    ...
    found = [
        d for d in r.json()["descriptors"] if d["ooni_run_link_id"] == ooni_run_link_id
    ]
    assert len(found) == 1

    say("list all items as admin")
    r = client_with_admin_role.get("/api/_/ooni_run/list")
    assert r.status_code == 200, r.json()
    assert r.json()["v"] == 1, r.json()
    assert sorted(r.json()) == ["descriptors", "v"]
    assert len(r.json()["descriptors"]) > 0
    ...
    r = client_with_user_role.post(
        f"/api/_/ooni_run/create?ooni_run_link_id={ooni_run_link_id}", json=z
    )
    assert r.status_code == 200, r.json()
    assert r.json()["v"] == 1, r.json()
    ...
    r = client_with_admin_role.get(f"/api/_/ooni_run/list?ids={ooni_run_link_id}")
    assert r.status_code == 200, r.json()
    ...
    r = client_with_user_role.get(
        f"/api/_/ooni_run/list?ids={ooni_run_link_id}&only_mine=True"
    )
    assert r.status_code == 200, r.json()
    descs = r.json()["descriptors"]
    assert len(descs) == 2, r.json()
    ...
    say("Archive it")
    r = client_with_user_role.post(f"/api/_/ooni_run/archive/{ooni_run_link_id}")
    assert r.status_code == 200, r.json()
    assert r.json()["v"] == 1, r.json()

    say("List descriptors")
    r = client_with_user_role.get(
        f"/api/_/ooni_run/list?ids={ooni_run_link_id}&include_archived=True"
    )
    assert r.status_code == 200, r.json()
    descs = r.json()["descriptors"]
    assert len(descs) == 2, r.json()
    ...
    say("List descriptors")
    r = client_with_user_role.get(f"/api/_/ooni_run/list?ids={ooni_run_link_id}")
    assert r.status_code == 200, r.json()
    descs = r.json()["descriptors"]
